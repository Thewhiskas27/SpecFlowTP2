...
{
    public static void addCandidat(Poll p, string name)
    {
        var c = new Candidat(name);
        p.candidats.Add(c);
    }
    public static int getAllVotes(Poll p)
    {
        int votes = 0;
        foreach (var c in p.candidats) { votes += c.votes;}
        return votes;
    }
    public static int getAllVotes(List<Candidat> cs)
    {
        int votes = 0;
        foreach (var c in cs) { votes += c.votes; }
        return votes;
    }
    public static List<PourcentVotes> getPercs(Poll p)
    {
    ...
    public static List<PourcentVotes> getPercs(List<Candidat> cs)
    {
        int votes = getAllVotes(cs);
        List<PourcentVotes> pv = new();
        foreach (var c in cs) { pv.Add(new PourcentVotes(c, c.votes * 100 / votes)); }
        return pv;
    }
    public static List<Candidat> getWinners(List<PourcentVotes> pv)
    {
